...
class EBlock(nn.Module):
    def __init__(self, out_channel, num_res=20):  # 8是普通版本，20是plus
...
class DBlock(nn.Module):
    def __init__(self, channel, num_res=20):
...
if __name__ == "__main__":
    ...
    input = torch.ones(1, 3, 224, 224, dtype=torch.float, requires_grad=False).cuda()

    model = MIMOUNetPlus().cuda()
    out = model(input)
    out_ = out[2]   # 巨坑
