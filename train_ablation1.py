...
from model.MDDA_hybrid import RCUNet
...
with open('options/Derain1.yaml', 'r') as config:
...
## Resume
if Train['RESUME']:
    path_chk_rest = utils.get_last_path(model_dir, '_latest_ablation.pth')  # bestPSNR or latest
...
for epoch in range(start_epoch, OPT['EPOCHS'] + 1):
    ...
    ## Validation
        if (i+1) % eval_now == 0 and epoch > 500:
            with torch.no_grad():
                model.eval()
                psnr_val_rgb = []
                ssim_val_rgb = []
                ...
                if psnr_val_rgb > best_psnr:
                    ...
                    torch.save({'epoch': epoch,
                                ...
                                'optimizer': optimizer.state_dict()}, os.path.join(model_dir, "model_bestPSNR_x.pth"))
    ...
    torch.save({'epoch': epoch,
                'state_dict': model.state_dict(),
                'optimizer': optimizer.state_dict()}, os.path.join(model_dir, "model_latest_x.pth"))
