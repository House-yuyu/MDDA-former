...
with open('options/Gaussian.yaml', 'r') as config:
...
## Resume
if Train['RESUME']:
    path_chk_rest = utils.get_last_path(model_dir, '_bestPSNR_gaussian.pth')  #
...
for epoch in range(start_epoch, OPT['EPOCHS'] + 1):
    ...
    ## Validation
        if (i+1) % eval_now == 0 and epoch > 50:
            with torch.no_grad():
                model.eval()
                psnr_val_rgb = []
                ssim_val_rgb = []
                ...
                if psnr_val_rgb > best_psnr:
                    ...
                    torch.save({'epoch': epoch,
                                ...
                                'optimizer': optimizer.state_dict()}, os.path.join(model_dir, "model_bestPSNR_gaussian.pth"))
    ...
    torch.save({'epoch': epoch,
                'state_dict': model.state_dict(),
                'optimizer': optimizer.state_dict()}, os.path.join(model_dir, "model_latest_gaussian.pth"))
