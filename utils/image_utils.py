...
def calculate_ssim(img1, img2, border=0):
    ...
    if img1.ndim == 2:
        return ssim_np(img1, img2)
    elif img1.ndim == 3:
        if img1.shape[2] == 3:
            ssims = []
            for i in range(3):
                ssims.append(ssim_np(img1[:, :, i], img2[:, :, i]))
            return np.array(ssims).mean()
        elif img1.shape[2] == 1:
            return ssim_np(np.squeeze(img1), np.squeeze(img2))
    else:
...
def calculate_rgb_psnr(img1, img2):  # useful for RGB test
