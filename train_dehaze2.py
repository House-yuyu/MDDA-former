...
import utils
from utils import losses, get_temperature, mkdir, Adan
...
with open('options/Dehaze2.yaml', 'r') as config:
...
train_haze = Train['train_hazy_path']
val_haze = Train['val_hazy_path']
...
if OPT['type'] == 'Adam':
    ...
    optimizer = Adan(model.parameters(), lr=lr_initial, weight_decay=args.weight_decay, betas=args.opt_betas, eps=args.opt_eps,
...
## Resume
if Train['RESUME']:
    path_chk_rest = utils.get_last_path(model_dir, '_latest_OTS.pth')  # bestPSNR or latest
...
train_dataset = TrainDataset(train_haze, train_clear)
val_dataset = ValDataset(val_haze, val_clear)
...
for epoch in range(start_epoch, OPT['EPOCHS'] + 1):
            ...
            with torch.no_grad():
                ...
                if psnr_val_rgb > best_psnr:
                    ...
                    torch.save({'epoch': epoch,
                                ...
                                'optimizer': optimizer.state_dict()}, os.path.join(model_dir, "model_bestPSNR_OTS.pth"))
                ...
                if ssim_val_rgb > best_ssim:
                    ...
                    torch.save({'epoch': epoch,
                                ...
                                }, os.path.join(model_dir, "model_bestSSIM_OTS.pth"))
    ...
    # Save the last model    以epoch为单位
    torch.save({'epoch': epoch,
                'state_dict': model.state_dict(),
                'optimizer': optimizer.state_dict()}, os.path.join(model_dir, "model_latest_OTS.pth"))
