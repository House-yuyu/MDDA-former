...
def main():
    parser = argparse.ArgumentParser(description="metrics test")
                                                              # Raindrop  // Rain13K/test
    parser.add_argument('--input_dir', default='../data/Derain/Rain13K/test', type=str, help='Directory of validation images')
    ...
    datasets = ['Rain100H']   # ['Rain100L', 'Rain100H', 'Test100', 'Test1200', 'Test2800']  //  'test_a' , 'test_b'
    ...
    for dataset in datasets:
        test_files_gt = natsorted(
            glob.glob(os.path.join(opt.input_dir, dataset, 'target',  '*.png'))
                      + glob.glob(os.path.join(opt.input_dir, dataset, 'target', '*.jpg')))
        test_files = natsorted(
            glob.glob(os.path.join(opt.result_dir, dataset, '*.png')))
    ...
    print("\nPSNR_Y_average %.4f \nSSIM_Y_average %.4f" % (Mean_PSNR, Mean_SSIM))
