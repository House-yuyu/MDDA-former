...
# from model.Test1123 import RCUNet
...
parser.add_argument("--batch_size", type=int, default=9, help="Batch size (default: %(default)s)")  # single
...
with open('options/SIDD.yaml', 'r') as config:
...
## Resume
if Train['RESUME']:
    path_chk_rest = utils.get_last_path(model_dir, '_bestPSNR.pth')  #
...
------------------------------------------------------------------
    ...
    Batch sizes:        {args.batch_size * world_size}
...
for epoch in range(start_epoch, OPT['EPOCHS'] + 1):
    ...
    ## Validation
        if (i+1) % eval_now == 0 and epoch > 20:
            with torch.no_grad():
                model.eval()
                psnr_val_rgb = []
                ssim_val_rgb = []
